module.exports = {
  ...
  create: function(context) {
    ...
    return {
       ...
       *
       * @TODO:
       * - [ ] add regex to match `secondFuncArgName` safely...
       * - [ ] !!! SUPPORT FOR `const firstFuncArgName = iteratee[i]`
       */
      'ExpressionStatement': function(node) {
        ...
        var firstFuncArg = funcParams[0]
        var secondFuncArg = funcParams[1]

        // might have no args
        var firstFuncArgName = null
        var secondFuncArgName = null
        if (firstFuncArg) {
          firstFuncArgName = firstFuncArg.name
        }
        if (secondFuncArgName) {
          secondFuncArg = secondFuncArg.name
        }
        ...
        context.report({
          ...
          fix: function(fixer) {
            ...
            // var body = txt.replace(new RegExp(firstFuncArgName, 'g'), variableBeingLooped + '[i]')
            var body = replaceDeclarationIfNeeded(bodyTxt, firstFuncArgName, variableBeingLooped)
            var bodySplit = loopVariableDefinedInBlock(firstFuncArgName, assignmentValue, variableBeingLooped, body, false)

            code += 'for (let i = 0' + cacheLengthInLoop + '; i < ' + varLength + '; i++) ' + bodySplit
